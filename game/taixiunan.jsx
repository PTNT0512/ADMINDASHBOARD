...
const RealisticDice = ({ value, isRolling, position, isRevealed, isDarkMode, isExploding }) => {
  const faceRotations = {
  ...
  const Face = ({ num, transform }) => (
    <div 
      className={`absolute w-full h-full border-2 flex flex-wrap p-1.5 shadow-[inset_0_0_12px_rgba(0,0,0,0.8)] rounded-md overflow-hidden transition-all duration-300
        ...
        ${isExploding ? 'brightness-[3] border-white shadow-[0_0_20px_#fff]' : ''}`}
  ...
  return (
    <div 
      className={`absolute transition-all duration-1000 cubic-bezier(0.34, 1.56, 0.64, 1) z-10 ${isExploding ? 'animate-dice-boom' : ''}`} 
      style={{ 
        left: `${position.x}%`, 
        top: `${position.y}%`, 
...
const App = () => {
  ...
  const [isExploding, setIsExploding] = useState(false); // Trạng thái nổ xúc xắc
  ...
  const startResultPhase = () => {
    ...
    setIsExploding(true); // Kích nổ xúc xắc
    setTimeout(() => setIsExploding(false), 1500); // Hiệu ứng nổ mạnh nhất kéo dài 1.5s
  ...
  const startNewSession = () => {
    ...
    setIsExploding(false); // Reset bowl state
    ...
    // setSession(s => s + 1); // Client không tự tăng session, chờ server đồng bộ
    setSessionBets([
  ...
  const handleBowlStart = (e) => {
    if (!isReadyToScratch || isScratchedDone || isBowlFading) return;
  ...
  const handleBowlMove = (e) => {
    ...
    if (dist > 160) { isDraggingBowl.current = false; setBowlOffset({ x: newX, y: newY }); setIsBowlFading(true); setTimeout(startResultPhase, 200); }
  };
  ...
  const handleBowlEnd = () => {
    ...
    if (match) { setBowlOffset({ x: parseFloat(match[1]), y: parseFloat(match[2]) }); }
  };
  ...
  useEffect(() => {
    ...
    const game = params.get('startgame');
    if (token && game === 'taixiunan') {
  ...
  useEffect(() => {
    ...
    const syncInterval = setInterval(async () => {
      try {
        const res = await fetch('http://localhost:4001/api/game/status?game=taixiunan');
        const data = await res.json();
        console.log(`[taixiunan] Client Phase: ${phase}, Server Phase: ${data.phase}, Server Time: ${data.timeLeft}, Session ID: ${data.sessionId}`);
        ...
        if (data.phase === 'RESULT' && phase === 'BETTING') startRollingPhase(data.dice);
        if (data.phase === 'BETTING' && phase !== 'BETTING') startNewSession();
      } catch (e) { console.error("Lỗi đồng bộ với máy chủ (taixiunan):", e); }
    }, 1000);
    return () => clearInterval(syncInterval);
  }, [phase, session, isDemoMode]);
  ...
  return (
    <div className={`flex flex-col h-screen w-full max-w-md mx-auto overflow-hidden relative select-none font-mono transition-colors duration-500
      ...
      <style>{`
        ...
        @keyframes dice-boom-persist { 
          0% { transform: scale(1); filter: brightness(1); } 
          20% { transform: scale(1.6); filter: brightness(5) contrast(2); } 
          100% { transform: scale(1.35); filter: brightness(1.6); } 
        }
        .animate-dice-boom { animation: dice-boom-persist 0.8s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards; }
      ...
      <div className="relative h-[250px] flex items-center justify-center perspective-1000">
        <div className={`relative w-64 h-64 flex items-center justify-center ${isRolling ? 'animate-bowl-shake' : ''}`}>
          ...
          <div className={`absolute inset-4 rounded-full overflow-hidden border shadow-[inset_0_0_50px_#000] ${isDarkMode ? 'bg-[#020617] border-sky-500/40' : 'bg-slate-200 border-sky-500/60'}`}>
             ...
             {dice.map((v, i) => (
                 <RealisticDice key={i} value={v} isRolling={isRolling} position={dicePositions[i]} isRevealed={isScratchedDone} isDarkMode={isDarkMode} isExploding={isExploding} />
             ))}
          </div>

          {/* BÁT CHỈ HUY - NẶN TỰ DO & TAN BIẾN KHI RA NGOÀI */}
          {!isScratchedDone && (phase === "BETTING" || isRolling || isReadyToScratch) && (
            <div 
                ref={bowlRef}
                className={`absolute inset-2 z-[100] transition-all duration-300
                    ${isDraggingBowl.current ? 'duration-0 scale-100 opacity-100 blur-0' : isBowlFading ? 'duration-1000 scale-125 opacity-0 blur-xl pointer-events-none' : 'duration-500'}`}
                style={{ transform: `translate(${bowlOffset.x}px, ${bowlOffset.y}px)`, touchAction: 'none', willChange: 'transform' }}
            >
               <div 
