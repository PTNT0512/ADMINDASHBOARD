    ...
    const syncInterval = setInterval(async () => {
      try {
        ...
        if (data.phase === 'BETTING' && phase !== 'BETTING') startNewSession();
          ...
          if (!isMounted) return;
          console.error("Lỗi đồng bộ với máy chủ (taixiunan):", e); 
          setIsDemoMode(true); // Chuyển sang chế độ Demo nếu mất kết nối
      }
    }, 1000);
    return () => { isMounted = false; clearInterval(syncInterval); };
  }, [phase, session, isDemoMode, isScratchedDone]);
          ...
          {!isScratchedDone && (phase === "BETTING" || isRolling || isReadyToScratch) && (
            <div 
                ...
                className={`absolute inset-2 z-[100] transition-all duration-300
                    ${isDraggingBowl.current ? 'duration-0 scale-100 opacity-100 blur-0' : isBowlFading ? 'duration-500 scale-110 opacity-0 blur-xl pointer-events-none' : 'duration-500 ease-out'}`}
