...
const AviatorGame = () => {
  ...
  // Refs để truy cập trong loop/socket callback
  const autoCashOutRef = useRef({ enabled: false, value: 2.00 });
  const betStatusRef = useRef({ placed: false, cashedOut: null });
  const handleCashOutRef = useRef(null);

  useEffect(() => {
    autoCashOutRef.current = { enabled: isAutoCashOutEnabled, value: autoCashOutValue };
    betStatusRef.current = { placed: isBetPlacedForCurrent, cashedOut: cashedOut };
    handleCashOutRef.current = handleCashOut;
  }, [isAutoCashOutEnabled, autoCashOutValue, isBetPlacedForCurrent, cashedOut]);
  ...
  useEffect(() => {
    ...
    socket.on('connect_error', () => {
        console.log("⚠️ Socket Error, switching to Demo Mode");
        setIsDemoMode(true);
    });

    socket.on('aviator_update', (data) => {
        if (data.phase === 'FLYING') {
            if (!isFlying) {
                setIsFlying(true);
                setIsCrashed(false);
                setCashedOut(null);
                if (canvasRef.current) initUniverse(canvasRef.current.width, canvasRef.current.height);
                startTimeRef.current = Date.now() - (data.elapsed || 0);
            }
            setMultiplier(data.multiplier);

            // Auto Cashout Check (Online)
            const { enabled, value } = autoCashOutRef.current;
            const { placed, cashedOut: isCashed } = betStatusRef.current;
            if (enabled && placed && !isCashed && data.multiplier >= value) {
                if (handleCashOutRef.current) handleCashOutRef.current(data.multiplier);
            }

            draw(data.multiplier, data.elapsed || 0);
        } else if (data.phase === 'CRASHED') {
            handleCrash(data.multiplier);
        } else if (data.phase === 'WAITING') {
            setIsFlying(false);
            setIsCrashed(false);
            setWaitTime(data.timeLeft);
            setMultiplier(1.00);
        }
    });

    return () => socket.disconnect();
  }, [isFlying]); // Dependency isFlying để đảm bảo logic init đúng
  ...
  const updateMultiplier = (time) => {
    ...
    // Auto Cashout Check (Demo)
    const { enabled, value } = autoCashOutRef.current;
    // Trong demo mode, betStatusRef cũng được cập nhật
    if (enabled && isBetPlacedForCurrent && !cashedOut && newMultiplier >= value) {
  ...
  useEffect(() => {
    if (!isDemoMode) return; // Nếu đang Online (Socket) thì không chạy logic đếm ngược local
    let timer;
  ...
  const handleBetClick = () => {
    ...
    if (!isFlying) {
        if (isBetPlacedForCurrent) {
            ...
            if (currentUser) {
                fetch('http://localhost:4001/api/game/cancel-bet', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        alert(data.message || "Hủy cược thất bại");
                    }
  ...
  const handleCashOut = (specificMultiplier = null) => {
    ...
    if (currentUser && !isDemoMode) {
        ...
        .then(data => {
            ...
            else console.error("Cashout failed:", data.message);
        })
  ...
  return (
    <div className="flex flex-col h-screen bg-[#020205] text-white font-sans overflow-hidden select-none safe-bottom">
      ...
      <div className="p-4 pt-6 pb-10 bg-[#08080c] border-t border-white/5 rounded-t-[45px] shadow-[0_-30px_80px_rgba(0,0,0,0.9)] z-40">
        ...
        <div className="flex items-center justify-between mb-4 px-3 py-2 bg-white/5 rounded-xl border border-white/5">
            <div className="flex items-center gap-3">
                <span className="text-[10px] font-bold text-white/60 uppercase tracking-wider">Tự động chốt</span>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input type="checkbox" className="sr-only peer" checked={isAutoCashOutEnabled} onChange={(e) => setIsAutoCashOutEnabled(e.target.checked)} disabled={isBetPlacedForCurrent || isBetPlacedForNext} />
            ...
            <div className="flex items-center gap-2">
                 ...
                 <input 
                    ...
                    disabled={!isAutoCashOutEnabled || isBetPlacedForCurrent || isBetPlacedForNext}
                    className="w-20 bg-black/40 border border-white/10 rounded-lg px-2 py-1 text-right font-mono font-bold text-sm text-white focus:outline-none focus:border-emerald-500/50 transition-colors"
                 />
            </div>
        </div>
