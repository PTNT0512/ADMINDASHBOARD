...
const host = String(process.env.CENTER_HOST || '127.0.0.1').trim() || '127.0.0.1';
const port = readInt(process.env.CENTER_PORT, 55174);
const waitHost = host === '0.0.0.0' ? '127.0.0.1' : host;
const killPorts = unique([String(port), '55174', '5174']);
const sharedEnv = {
  ...process.env,
...
viteProc.on('exit', (code) => {
  if (!shuttingDown) {
    console.error('[center-dev] vite exited with code ' + code);
...
if (!rendererOnly) {
  waitForUrl('http://' + waitHost + ':' + port + '/@vite/client').then(() => {
    console.log('[center-dev] renderer ready -> http://' + waitHost + ':' + port);
    const electronEnv = { ...sharedEnv };
    delete electronEnv.ELECTRON_RUN_AS_NODE;

    const electronProc = spawnTool(electronBin, ['electron/center.cjs'], { env: electronEnv });
    ...
    console.error('[center-dev] renderer not ready:', error && error.message ? error.message : error);
    stopChildren(1);
  });
}
