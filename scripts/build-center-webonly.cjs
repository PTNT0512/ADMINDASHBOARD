...
const outdir = path.join(root, 'dist-center');
const mainEntry = path.join(root, 'src', 'main.jsx');
const centerAppEntry = path.join(root, 'src', 'App.center.jsx');
...
(async () => {
  ...
  const result = await esbuild.build({
    ...
    define: {
      'import.meta.env.VITE_APP_MODE': JSON.stringify('center'),
      'process.env.NODE_ENV': JSON.stringify('production'),
    },
    plugins: [
      {
        name: 'alias-center-app-entry',
        setup(build) {
          build.onResolve({ filter: /^@app-root$/ }, () => ({ path: centerAppEntry }));
        },
      },
    ],
  });
  ...
  if (!entryChunk) {
    throw new Error('Cannot find compiled main entry chunk for Center web build.');
  }
  ...
  console.log(`[center-web-build] built ${jsRelative}${cssRelative ? ` and ${cssRelative}` : ''}`);
})();
