...
function bufferFromAny(data) {
  ...
  throw new Error("Unsupported message type. Expected Buffer, Uint8Array, or ArrayBuffer.");
}

function buildClientPacket({ controllerId = 1, cmdId, payload = Buffer.alloc(0), isCompress = false }) {
  if (typeof cmdId !== "number") {
    throw new Error("cmdId must be a number.");
  }
  const body = Buffer.alloc(1 + 2 + payload.length);
  body.writeUInt8(controllerId & 0xff, 0);
  body.writeInt16BE(cmdId, 1);
  payload.copy(body, 3);

  const isBigSize = body.length > 0xffff;
  const headerSize = isBigSize ? 5 : 3;
  const packet = Buffer.alloc(headerSize + body.length);
  packet.writeUInt8(genHeader(isBigSize, isCompress), 0);
  if (isBigSize) {
...
function stripPacketHeader(rawBuffer) {
  ...
  const looksLikeFramed =
    getBit(header, BIT_IS_BINARY_INDEX) && getBit(header, BIT_IS_BLUE_BOXED_INDEX);
  ...
  const headerSize = isBigSize ? 5 : 3;
  if (rawBuffer.length < headerSize) {
    return rawBuffer;
  }
...
function parseServerPacket(data) {
