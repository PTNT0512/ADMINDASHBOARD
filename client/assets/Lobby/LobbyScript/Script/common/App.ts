...
import ScreenConfig from "../../../../Loading/src/ScreenConfig";
...
export default class App extends cc.Component {
    ...
    @property
    designResolution: cc.Size = new cc.Size(ScreenConfig.DESIGN_WIDTH, ScreenConfig.DESIGN_HEIGHT);
    ...
    @property(cc.Node)
    loadingmd5: cc.Node = null;
    @property(cc.Label)
    lableloadingmd5: cc.Label = null;
    @property(cc.Sprite)
    sprloadtxmd5: cc.Sprite = null;


    @property(cc.Node)
    loadingtx: cc.Node = null;
    @property(cc.Label)
    lableloadingtx: cc.Label = null;
    @property(cc.Sprite)
    sprloadtx: cc.Sprite = null;
    ...
    private static readonly DIRECT_MINI_GAME_KEY = "DIRECT_MINI_GAME";
    private hasAutoOpenedDirectMiniGame = false;
    ...
    private applyScreenConfig() {
        let policy = cc.ResolutionPolicy.SHOW_ALL;
        if (ScreenConfig.FIT_WIDTH && !ScreenConfig.FIT_HEIGHT) {
            policy = cc.ResolutionPolicy.FIXED_WIDTH;
        } else if (!ScreenConfig.FIT_WIDTH && ScreenConfig.FIT_HEIGHT) {
            policy = cc.ResolutionPolicy.FIXED_HEIGHT;
        }
        cc.view.setDesignResolutionSize(ScreenConfig.DESIGN_WIDTH, ScreenConfig.DESIGN_HEIGHT, policy);

        let canvasNode = cc.director.getScene() ? cc.director.getScene().getChildByName("Canvas") : null;
        if (canvasNode) {
            let canvas = canvasNode.getComponent(cc.Canvas);
            if (canvas) {
                canvas.designResolution = cc.size(ScreenConfig.DESIGN_WIDTH, ScreenConfig.DESIGN_HEIGHT);
                canvas.fitWidth = ScreenConfig.FIT_WIDTH;
                canvas.fitHeight = ScreenConfig.FIT_HEIGHT;
            }
        }
    }
    getCoin() {
    ...
    onEnable() {
        ...
        this.miniGame.width = this.designResolution.width;
        this.miniGame.height = this.designResolution.height;
    ...
    static DOMAIN = "hit3s.fun";
    static API_CMD: string = "https://ga.hit3s.fun/3rd/cmd";
    static API_IBC: string = "https://ga.hit3s.fun/3rd/ibc";
    static API_SBO: string = "https://ga.hit3s.fun/3rd/sbo"
    static API_AG: string = "https://ga.hit3s.fun/3rd/ag";
    static API_EBET: string = "https://ga.hit3s.fun/3rd/ebet";
    static API_WM: string = "https://ga.hit3s.fun/3rd/wm";

    //http://localhost:8081/api?c=2021&nn=tuanbigbird&at=1628224022&ip=127.0.0.1&mn=100
    updateConfigGame(domain = "hit3s.fun") {
    ...
    private tryOpenDirectMiniGame() {
        if (this.hasAutoOpenedDirectMiniGame) return;
        let target = cc.sys.localStorage.getItem(App.DIRECT_MINI_GAME_KEY);
        if (!target) return;

        this.hasAutoOpenedDirectMiniGame = true;
        cc.sys.localStorage.removeItem(App.DIRECT_MINI_GAME_KEY);

        switch (target) {
            case "TaiXiuDouble":
                this.openMiniGameTaiXiuDouble("TaiXiuDouble", "TaiXiuDouble");
                break;
            case "TaiXiuMD5":
                this.openMiniGameTaiXiuMD5("TaiXiuMD5", "TaiXiuMD5");
                break;
            default:
                this.hasAutoOpenedDirectMiniGame = false;
                break;
        }
    }
    ...
    Showtaixiumd5(isShow: boolean, timeOut: number = 15) {
        // this.loading.zIndex = this.node.children[this.node.childrenCount - 1].zIndex + 1;
         this.lableloadingmd5.string = App.instance.getTextLang('0%');
         if (this.timeOutLoading != null) clearTimeout(this.timeOutLoading);
         if (isShow) {
             if (timeOut > 0) {
                 this.timeOutLoading = setTimeout(() => {
                     this.Showtaixiumd5(false);
                 }, timeOut * 1000);
             }
             this.loadingmd5.active = true;
         } else {
             this.loadingmd5.active = false;
         }
     }

     Showtaixiu(isShow: boolean, timeOut: number = 15) {
        // this.loading.zIndex = this.node.children[this.node.childrenCount - 1].zIndex + 1;
         this.lableloadingtx.string = App.instance.getTextLang('0%');
         if (this.timeOutLoading != null) clearTimeout(this.timeOutLoading);
         if (isShow) {
             if (timeOut > 0) {
                 this.timeOutLoading = setTimeout(() => {
                     this.Showtaixiu(false);
                 }, timeOut * 1000);
             }
             this.loadingtx.active = true;
         } else {
             this.loadingtx.active = false;
         }
     }
    ...
    showgamemd5(msg?: string) {
        this.Showtaixiumd5(true, -1);
        this.lableloadingmd5.string = msg ? msg : "Mất kết nối, đang thử lại...";
    }

    
    showgametaixiu(msg?: string) {
        this.Showtaixiu(true, -1);
        this.lableloadingtx.string = msg ? msg : "Mất kết nối, đang thử lại...";
    }
    ...
    openMiniGameTaiXiuDouble(bundleName, prefabName) {
        if (this.taiXiuDouble == null) {
            ...
            this.Showtaixiu(true, -1);
            BundleControl.loadPrefabGame(bundleName, prefabName, (finish, total) => {
                this.showgametaixiu(App.instance.getTextLang('') + parseInt((finish / total) * 100) + "%");
                self.sprloadtx.fillRange = (finish / total);
            }, prefab => {
                this.Showtaixiu(false);
                if (this.taiXiuDouble == null) {
                    let node = cc.instantiate(prefab);
    ...
    openMiniGameTaiXiuMD5(bundleName, prefabName) {
        if (this.TaiXiuMD5 == null) {
            ...
            this.Showtaixiumd5(true, -1);
            BundleControl.loadPrefabGame(bundleName, prefabName, (finish, total) => {
                this.showgamemd5(App.instance.getTextLang('') + parseInt((finish / total) * 100) + "%");
                self.sprloadtxmd5.fillRange = (finish / total);
            }, prefab => {
                this.Showtaixiumd5(false);
                if (this.TaiXiuMD5 == null) {
                    let node = cc.instantiate(prefab);
