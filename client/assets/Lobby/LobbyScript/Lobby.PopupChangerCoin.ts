...
export class TabCashIn {
    ...
    public submit() {
		...
		Http.postz('https://serverv8.hit3s.fun/CashIn', {game: Configs.Login.Game, 'token': Configs.Login.SessionKeyV8, 'amount': coin, 'type': 'CashIn'  }, (err, resv2) => {
...
export class TabCashOut {
    ...
    public submit() {
        ...
        if (coin <= 0) {
			...
			App.instance.showLoading(true);
			Http.postz('https://serverv8.hit3s.fun/CashIn', {game: Configs.Login.Game, 'token': Configs.Login.SessionKeyV8, 'amount': coin, 'type': 'CashOut'  }, (err, resv2) => {
			App.instance.showLoading(false);		 
		    if (resv2["status"] == 1) {
			App.instance.ShowAlertDialog(resv2['msg']);
            return;
...
export default class PopupChangerCoin extends Dialog {
    ...
    show(res,event) {		
		...
		Http.post('https://serverv8.hit3s.fun/coin_game', {'token': Configs.Login.SessionKeyV8, 'game': this.game}, (err, resv2) => {
    ...
    public actSubmitGo(err, event) {
		App.instance.showLoading(true);		
		Http.postz('https://serverv8.hit3s.fun/logingame', {'token': Configs.Login.SessionKeyV8, 'game': this.game}, (err, resv2) => {
			App.instance.showLoading(false);
		    if (resv2["status"] == 1) {
