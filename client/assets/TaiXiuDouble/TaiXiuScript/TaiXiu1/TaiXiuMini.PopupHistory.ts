...
namespace taixiumini {
    ...
    export class PopupHistory extends Dialog {
        ...
        private preparePopupHost() {
            const canvas = App.instance && App.instance.canvas ? App.instance.canvas : null;
        ...
        private applyResponsiveScale() {
            ...
            const size = this.container.getContentSize();
            if (!size || size.width <= 0 || size.height <= 0) return;
            const maxWidth = cc.winSize.width * 0.96;
            const maxHeight = cc.winSize.height * 0.92;
            const fitScale = Math.min(1, maxWidth / size.width, maxHeight / size.height);
            this.showScale = fitScale;
            this.startScale = Math.max(0.45, fitScale * 0.7);
        }
        ...
        private loadData(isReloadScr = true) {
            ...
            Http.get(Configs.App.API, { "c": 100, "p": this.page, "un": Configs.Login.Nickname, "mt": Configs.App.MONEY_TYPE, "txType": 1, "game": "double" }, (err, res) => {
