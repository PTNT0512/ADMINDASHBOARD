...
export default class TaiXiuMiniController extends cc.Component {
    ...
    private hasShownRefundAtFive = false;
    private readonly lockBetAtSecond = 5;
    private readonly refundNoticeAtSecond = 1;
        ...
        for (var i = 0; i < this.contentChatNhanh.childrenCount; i++) {
            ...
            node.on('click', function () {
                if (node && node.childrenCount > 0) {
                    ...
                    if (label) {
                        self.sendChat(label.string);
                    }
                }
                if (self.scrollChat) self.scrollChat.active = true;
                if (self.chatNhanh) self.chatNhanh.active = false;
            })
        }
    }

    toggleChatNhanh() {
        if (!this.chatNhanh || !this.scrollChat) {
            return;
        }
    ...
    start() {
            ...
            switch (inpacket.getCmdId()) {
                ...
                case cmd.Code.GAME_INFO: {
                    ...
                    if (res.bettingState) {
                        ...
                        const lockedBetting = remainBet <= this.lockBetAtSecond;
                        if (!lockedBetting) {
                            this.hasShownRefundAtFive = false;
                        }
                        if (lockedBetting && remainBet === this.refundNoticeAtSecond && !this.hasShownRefundAtFive) {
                            this.hasShownRefundAtFive = true;
                            this.showToast(this.getTextLangSafe('txt_taixiu_refund', "Tra tien can cua"));
                        }
                        this.isBetting = !lockedBetting;
                    ...
                    const remainBetForBalance = this.normalizeBetRemainTime(res.remainTime);
                    if (!res.bettingState || (res.bettingState && remainBetForBalance <= this.lockBetAtSecond)) {
                ...
                case cmd.Code.UPDATE_TIME: {
                    ...
                    if (res.bettingState) {
                        ...
                        if (res.remainTime == 60) {
                            this.showToast(this.getTextLangSafe('txt_taixiu_new_session', "Bat dau phien moi"));
                        }
                        this.isBetting = !lockedBetting;
                        this.lblRemainTime.node.active = true;
                        this.lblRemainTime.string = this.formatRemain(remainBet);
                        ...
                        if (remainResult < 5 && this.isNan && !this.isOpenBowl) {
                            this.bowl.active = false;
                            this.showResult();
                ...
                case cmd.Code.DICES_RESULT: {
                    ...
                    this.arrTimeoutDice.push(setTimeout(() => {
                        ...
                        this.dice3.setCompleteListener(() => {
                            this.lblRemainTime2.node.parent.active = true;
                            this.lblRemainTime2.node.active = true;
                            const remainResult = this.normalizeResultRemainTime(this.remainTime);
                            this.lblRemainTime2.string = remainResult < 10 ? "0" + remainResult : "" + remainResult;
                            if (!this.isNan) {
                                this.showResult();
                ...
                case cmd.Code.REFUND: {
                    ...
                    this.showToast(this.getTextLangSafe('txt_taixiu_refund1', "Phien da hoan tra"));
                    break;
                }
        ...
        this.bowl.on(cc.Node.EventType.TOUCH_MOVE, (event: cc.Event.EventTouch) => {
            if (!this.isNan || this.isBetting || !this.bowl.active) return;
    ...
    show() {
        ...
        this.layoutBet.active = false;
        this.layoutBet.y = 28;
        if (this.lblBetTai) this.lblBetTai.string = "ĐẶT CƯỢC";
        if (this.lblBetXiu) this.lblBetXiu.string = "ĐẶT CƯỢC";
    ...
    actBetTai() {
        ...
        if (this.layoutBet1) this.layoutBet1.active = true;
        if (this.bettai) this.bettai.active = false;
        if (this.betxiu) this.betxiu.active = true;
    }
    ...
    actBetXiu() {
        ...
        if (this.layoutBet1) this.layoutBet1.active = true;
        if (this.bettai) this.bettai.active = true;
        if (this.betxiu) this.betxiu.active = false;
    }

    actOtherNumber() {
        App.instance.showBgMiniGame("TaiXiu");
        if (this.layoutBet1) this.layoutBet1.active = false;
    }
    ...
    private showResult() {
        ...
        if (this.eftai && this.eftai.node && this.efxiu && this.efxiu.node) {
            this.eftai.node.active = true;
            this.efxiu.node.active = false;
            if (this.lastScore >= 11) {
                this.eftai.setAnimation(0, "tai", true);
                const taiText = this.eftai.node.parent && this.eftai.node.parent.getChildByName("text");
                const xiuText = this.efxiu.node.parent && this.efxiu.node.parent.getChildByName("text");
                if (taiText) taiText.active = false;
                if (xiuText) xiuText.active = true;
            } else {
                ...
                const xiuText = this.efxiu.node.parent && this.efxiu.node.parent.getChildByName("text");
                const taiText = this.eftai.node.parent && this.eftai.node.parent.getChildByName("text");
                if (xiuText) xiuText.active = false;
                if (taiText) taiText.active = true;
            }
        }
        if (this.effect && this.effect.node) {
    ...
    private stopWin() {
        if (this.eftai && this.eftai.node) {
            this.eftai.node.active = false;
            if (this.eftai.node.parent) {
                const taiText = this.eftai.node.parent.getChildByName("text");
                if (taiText) taiText.active = true;
            }
        }
        if (this.efxiu && this.efxiu.node) {
            this.efxiu.node.active = false;
            if (this.efxiu.node.parent) {
                const xiuText = this.efxiu.node.parent.getChildByName("text");
                if (xiuText) xiuText.active = true;
            }
        }
    ...
    private getTextLangSafe(key: string, fallback: string = ""): string {
        if (!App.instance || typeof App.instance.getTextLang !== "function") {
            return fallback;
        }
        const text = App.instance.getTextLang(key);
        if (!text || text === key || text.indexOf("txt_") === 0) {
            return fallback;
        }
        return text;
    }
