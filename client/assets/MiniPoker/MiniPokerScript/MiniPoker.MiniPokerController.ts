...
export default class MiniPokerController extends MiniGame {
    ...
    start() {
        ...
        for (let i = 0; i < this.buttonBets.length; i++) {
            ...
            btn.button.node.on("click", () => {
                ...
                MiniGameNetworkClient.getInstance().sendCheck(new cmd.SendChangeRoom(oldIdx, this.betIdx));
            });
        }
    ...
    show() {
        if (this.node.active && this.hasShownOnce) {
            this.reOrder();
            return;
        }
        if (!this.node.active) {
        ...
        MiniGameNetworkClient.getInstance().sendCheck(new cmd.SendScribe(this.betIdx));
    }
    ...
    actSpin() {
        ...
        MiniGameNetworkClient.getInstance().sendCheck(new cmd.SendSpin(this.listBet[this.betIdx]));
    }
