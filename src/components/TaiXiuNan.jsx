          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-6 gap-y-4 flex-1 overflow-hidden">
                ...
                <div className="bg-slate-900/5 p-4 rounded-xl border border-slate-700 flex flex-col gap-3">
                    ...
                    <div className="flex justify-center gap-4">
                        {dice.map((val, i) => (
                            ...
                            >
                            <DiceIcon value={val} className="text-slate-900" size={36} strokeWidth={2.5} />
                            </button>
                        ))}
                    </div>

                    {/* Các nút chọn nhanh - Đã chỉnh thành Grid 2 cột thống nhất */}
            ...
            <div className="flex flex-col gap-4 h-full overflow-hidden">
                ...
                <div className="bg-slate-900/5 rounded-xl border border-slate-700 overflow-hidden flex-1 flex flex-col">
                    <div className="p-3 bg-slate-8000 border-b border-slate-700 flex items-center gap-2 text-xs font-bold text-slate-300 uppercase bg-gradient-to-r from-slate-800 to-slate-900">
                                ...
                                {info.detailedBets && info.detailedBets.map((bet, idx) => {
                                    ...
                                    return (
                                        <tr key={idx} className={`hover:bg-white/5 transition-colors ${pStatus === 'kill' ? 'bg-red-900/20' : pStatus === 'nurture' ? 'bg-green-900/20' : ''}`}>
                                            ...
                                            <td className="p-2">
                                                {/* ĐÃ CHỈNH SỬA: Grid 2x2 cho các nút thao tác người chơi */}
...
const TaiXiuNan = () => {
    ...
    const toggleAi = async (key, enable) => {
        setGameState(prev => ({...prev, info: {...prev.info, isAiMode: enable}}));
        await callApi('toggle-ai', { enable });
    }
    
    const toggleAutoKill = async (key, enable) => {
        setGameState(prev => ({...prev, info: {...prev.info, isAutoKillMode: enable}}));
        await callApi('toggle-auto-kill', { enable });
    }

    const setPlayerStatus = async (key, playerId, status) => {
        setGameState(prev => ({...prev, info: {...prev.info, playerControl: {...prev.info.playerControl, [playerId]: status}}}));
        await callApi('player-control', { userId: playerId, status });
    }
